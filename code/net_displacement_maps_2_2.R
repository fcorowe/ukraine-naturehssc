...
file_path_shp_raions <- file.path("gadm41_UKR_shp", "gadm41_UKR_2.shp")
file_path_shp_oblasts <- file.path("gadm41_UKR_shp", "gadm41_UKR_1.shp")
...
ukr_shp_obj_oblasts$NAME_1_v2 <- gsub("'", "", ukr_shp_obj_oblasts$NAME_1) 
...
# Apply replacements
ukr_shp_obj_oblasts$NAME_1_v2 <- ifelse(
  ukr_shp_obj_oblasts$NAME_1_v2 %in% names(replacements),
  replacements[ukr_shp_obj_oblasts$NAME_1_v2],
  ukr_shp_obj_oblasts$NAME_1_v2
)
...
plot_final_map <- function(active_month) {
  ...
  net_migration_plot <- filtered_sf_object %>%
    ...
    coord_sf(datum = NA) +
    geom_sf(data = ukr_shp_obj_oblasts, color = "white", fill = "grey70", alpha = 0.2, linewidth = 1) +
    geom_sf_text(data = ukr_shp_obj_oblasts, aes(label = NAME_1_v2), colour = "grey35", check_overlap = F, size = 5.5)
  
  
  bar_legend <- filtered_sf_object %>%
    ggplot(aes(x = class_ranges)) +
    geom_bar(aes(fill = class_ranges), show.legend = FALSE) +
    scale_fill_manual(values = color_palette, guide = "none", drop = FALSE) +
...
png(filename = "/Users/franciscorowe/Dropbox/Francisco/Research/in_progress/ukraine-displacement/github/ukraine-pop-displacement/manuscript/figures/2_2/map_february.png", units="in", width=14, height=10, res=300)
  plot_final_map(2)
dev.off()

png(filename = "/Users/franciscorowe/Dropbox/Francisco/Research/in_progress/ukraine-displacement/github/ukraine-pop-displacement/manuscript/figures/2_2/map_may.png", units="in", width=14, height=10, res=300)
  plot_final_map(5)
dev.off()

png(filename = "/Users/franciscorowe/Dropbox/Francisco/Research/in_progress/ukraine-displacement/github/ukraine-pop-displacement/manuscript/figures/2_2/map_august.png", units="in", width=14, height=10, res=300)
  plot_final_map(8)
dev.off()
