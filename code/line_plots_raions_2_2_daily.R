...
adjusted_pop_sizes_for_raions_1 <- adjusted_pop_sizes_for_raions %>%
  group_by(NAME_1, NAME_2, month, day) %>%
...
interpolated_data <- adjusted_pop_sizes_for_raions_1 %>%
  filter(NAME_2 != "n.a. ( 181)" & month != 2) %>% 
  group_by(NAME_1_v2, NAME_2, month) %>%
  reframe(
    day_interpolated = lin_interp(day, day),
    median_daily_change_interpolated = lin_interp(day, median_daily_change)
  )

# Create a grouping variable for each positive/negative segment
cat.rle = rle(interpolated_data$median_daily_change_interpolated < 0)
...
interpolated_data$month <- factor(
  interpolated_data$month,
  labels = c(
...
plot_line_plots <- function(interpolated_data_2, winners_losers) {
  result <- ggplot(
    interpolated_data_2,
    aes(x = fct_inorder(paste(month, day_interpolated, sep = " ")), 
        y = median_daily_change_interpolated,
        group = group
        )
    ...
    geom_ribbon(
      aes(
        ymin = pmin(median_daily_change_interpolated, 0),
        ymax = pmax(median_daily_change_interpolated, 0),
        fill = as.factor(net_outcome)
      ),
      alpha = 1,
      position = "identity",
      show.legend = FALSE
    ) +
    labs(x = "Day", y = "Median Net Migration") +
    scale_y_continuous(
      labels = function(x)
